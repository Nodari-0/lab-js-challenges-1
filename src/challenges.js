...
const howManyTimes = (arrOfStr, wordToCheck) =>
  arrOfStr.length === 0
    ? 0
    : arrOfStr.filter((word) => word === wordToCheck).length;
...
const filterOut = (arrayOriginal, toRemoveArray) => {
  if (arrayOriginal.length === 0) return null;
  return arrayOriginal.filter((item) => !toRemoveArray.includes(item));
};
...
const uniquifyArray = (uniqueArray) => {
  if (uniqueArray.length === 0) return null;
  return uniqueArray.reduce((uniqueItems, current) => {
